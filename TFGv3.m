...
% Altere aqui caso não deseje utilizar a tabela em csv
...
for ind = 1:tamanhotabela(1)
    ...
    Objeto(idxobj).idadeMax = todaspecas{ind,3};
    Objeto(idxobj).custo = todaspecas{ind,4};
    Objeto(idxobj).quantidade = todaspecas{ind,5};
    Objeto(idxobj).qtdnecessaria = todaspecas{ind,6};
    Objeto(idxobj).qualidade = todaspecas{ind,7};
    Objeto(idxobj).horahomem = todaspecas{ind,8};
    Objeto(idxobj).k1 = todaspecas{ind,9};
    Objeto(idxobj).k2 = todaspecas{ind,10};
    Objeto(idxobj).k3 = todaspecas{ind,11};
    Objeto(idxobj).atualiza;
    Objeto(idxobj).atualizaexibir;
...
if flagPARADA == 0
    ...
    linha2 = "Houve parada TOTAL, perda da produção em R$ " + handles.valorPARADATOTAL;
end
todaslinhas = { linha0
                linha1
                linha2};
